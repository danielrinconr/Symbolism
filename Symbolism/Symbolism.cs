...
namespace Symbolism
{
  ...
  public class Equation : MathObject
  {
    ...
    public override bool Equals(object obj)
    {
      return
          obj is Equation &&
          a.Equals(((Equation) obj).a) &&
          b.Equals(((Equation) obj).b) &&
          Operator == ((Equation) obj).Operator;
    }
    bool ToBoolean()
    {
      ...
      switch (eq.Operator)
      {
        ...
        case Operators.LessThan:
          if (eq.a is Number && eq.b is Number)
            return ((Number) eq.a).ToDouble().val < ((Number) eq.b).ToDouble().val;
          break;
      }
      if (eq.Operator != Operators.GreaterThan) throw new Exception();
      if (eq.a is Number && eq.b is Number)
        return ((Number) eq.a).ToDouble().val > ((Number) eq.b).ToDouble().val;
      throw new Exception();
    }
  ...
  public class DoubleFloat : Number
  {
    ...
    public override bool Equals(object obj)
    {
      if (obj is DoubleFloat && tolerance.HasValue)
        return Math.Abs(val - ((DoubleFloat) obj).val) < tolerance;
      return val == (obj as DoubleFloat)?.val;
    }
    public override int GetHashCode()
    { return val.GetHashCode(); }
  ...
  public class Fraction : Number
  {
    ...
    public override DoubleFloat ToDouble() => new DoubleFloat(numerator.val / (double)denominator.val);
    //////////////////////////////////////////////////////////////////////
    public override bool Equals(object obj) =>
  ...
  public static class Rational
  {
    ...
    public static MathObject SimplifyRationalNumber(MathObject u)
    {
      ...
      if (!(u is Fraction)) throw new Exception();
      var u_ = (Fraction)u;
    ...
    public static MathObject SimplifyRNERec(MathObject u)
    {
      while (true)
      {
        if (u is Integer) return u;
        if (u is Fraction)
          return Denominator((Fraction) u).val == 0 ? new Undefined() : u;
        ...
        {
          v = SimplifyRNERec(((Difference) u).elts[0]);
          return v == new Undefined() ? v : EvaluateProduct(new Integer(-1), v);
        }
        if (u is Sum && ((Sum) u).elts.Count == 2)
        {
          v = SimplifyRNERec(((Sum) u).elts[0]);
        ...
        {
          v = SimplifyRNERec(((Product) u).elts[0]);
        ...
        //}
        if (!(u is Power)) throw new Exception();
        v = SimplifyRNERec(((Power) u).bas);
        return v == new Undefined() ? v : EvaluatePower(v, ((Integer) ((Power) u).exp).val);
      }
    }
  ...
  public class Symbol : MathObject
  {
    ...
    public override bool Equals(object obj)
    {
      if (obj is Symbol) return name == ((Symbol) obj).name;
      return false;
    }
  }
  ...
  public static class ListUtils
  {
    ...
    public static bool equal(List<MathObject> a, List<MathObject> b)
    {
      ...
      return a[0] == b[0] && equal(a.Cdr(), b.Cdr());
    }
  }
  public class Function : MathObject
  {
    ...
    public MathObject Simplify()
    {
      return proc != null ? proc(args.ToArray()) : this;
    }
  ...
  public class And : Function
  {
    ...
    {
      if (!ls.Any()) return true;
      if (ls.Count() == 1) return ls.First();
      if (ls.Any(elt => elt == false)) return false;
      if (ls.Any(elt => elt == true))
        return new And { args = new List<MathObject>(ls.Where(elt => elt != true)) }.Simplify();
      if (!ls.Any(elt => elt is And)) return new And {args = new List<MathObject>(ls)};
      var and = new And();
    ...
    {
      var ls = new List<MathObject>(args) {obj};
      var and = new And {args = ls};
      return and.Simplify();
    }
  }
  public class Or : Function
  {
    MathObject OrProc(params MathObject[] ls)
    {
      ...
      if (ls.Any(elt => elt == false))
        return new Or { args = ls.Where(elt => elt != false).ToList() }.Simplify();
      if (ls.Any(elt => (elt is Bool) && ((Bool) elt).val)) return new Bool(true);
      if (ls.All(elt => (elt is Bool) && ((Bool) elt).val == false)) return new Bool(false);
      if (!ls.Any(elt => elt is Or)) return new Or {args = new List<MathObject>(ls)};
      var or = new Or();
      foreach (var elt in ls)
      {
  ...
  public static class OrderRelation
  {
    ...
    {
      if ((u as Product)?.elts[0] is Number)
    ...
    {
      if (!(u is Product)) return new Integer(1);
      var res = (Product)u;
    ...
    public static bool Compare(MathObject u, MathObject v)
    {
      while (true)
      {
        ...
        if (u is DoubleFloat && v is Fraction) return ((DoubleFloat) u).val < ((Fraction) v).numerator.val/((double) ((Fraction) v).denominator.val);
        if (u is Integer && v is DoubleFloat) return ((Integer) u).val < ((DoubleFloat) v).val;
        if (u is Fraction && v is DoubleFloat) return ((Fraction) u).numerator.val/((double) ((Fraction) u).denominator.val) < ((DoubleFloat) v).val;
        if (u is Integer)
        {
          u = new Fraction((Integer) u, new Integer(1));
          continue;
        }
        if (v is Integer)
        {
          v = new Fraction((Integer) v, new Integer(1));
          continue;
        }
        if (u is Fraction && v is Fraction)
        {
          ...
          //
          // (a d) / (b d)   <   (c b) / (b d)
          return (u_.numerator.val*v_.denominator.val) < (v_.numerator.val*u_.denominator.val);
        }
        if (u is Symbol && v is Symbol)
          return string.CompareOrdinal(((Symbol) u).name, ((Symbol) v).name) < 0;
        ...
        if (u is Function && v is Function)
        {
          var u_ = (Function) u;
          var v_ = (Function) v;
          return u_.name == v_.name ? O3(u_.args, v_.args) : string.CompareOrdinal(u_.name, v_.name) < 0;
        }
        if ((u is Fraction || u is DoubleFloat))
          return true;
        if (u is Product && (v is Power || v is Sum || v is Function || v is Symbol))
        {
          v = new Product(v);
          continue;
        }
        ...
        if (u is Function && v is Symbol)
        {
          var u_ = (Function) u;
          var v_ = (Symbol) v;
          if (u_.name == v_.name) return false;
          u = new Symbol(u_.name);
          continue;
        }
        return !Compare(v, u);
      }
  ...
  public class Power : MathObject
  {
    ...
    public MathObject Simplify()
    {
      ...
      if (!(v is Product) || !(w is Integer)) return new Power(v, w);
  ...
  public class Product : MathObject
  {
    ...
    //////////////////////////////////////////////////////////////////////
    private static List<MathObject> MergeProducts(List<MathObject> pElts, List<MathObject> qElts)
    {
      while (true)
      {
        ...
        switch (res.Count)
        {
          case 0:
            ...
            continue;
          case 1:
            return MergeProducts(ps, qs).Cons(res[0]);
        }
    ...
    static List<MathObject> SimplifyDoubleNumberProduct(DoubleFloat a, Number b)
    {
      ...
      return val == 1.0 ? new List<MathObject>() : List<MathObject>(new DoubleFloat(val));
    }
    public static List<MathObject> RecursiveSimplify(List<MathObject> elts)
    {
      if (elts.Count == 2)
      {
            ...
            (elts[1] is Integer || elts[1] is Fraction))
        {
          ...
          return P == 1 ? new List<MathObject>() : List(P);
        }
        ...
        if (OrderRelation.Base(p) != OrderRelation.Base(q))
          return OrderRelation.Compare(q, p) ? List(q, p) : List(p, q);
        var res = OrderRelation.Base(p) ^ (OrderRelation.Exponent(p) + OrderRelation.Exponent(q));
        return res == 1 ? new List<MathObject>() : List(res);
      }
    ...
    public MathObject Simplify()
    {
      ...
      return res.Any(elt => elt is Product) ? new Product { elts = res }.Simplify() : new Product { elts = res };
    }
  }
  public class Sum : MathObject
  {
    ...
    //////////////////////////////////////////////////////////////////////
    private static List<MathObject> MergeSums(List<MathObject> pElts, List<MathObject> qElts)
    {
      while (true)
      {
        if (pElts.Count == 0) return qElts;
        ...
        switch (res.Count)
        {
          case 0:
            ...
            continue;
          case 1:
            return MergeSums(ps, qs).Cons(res[0]);
        }
    ...
    static List<MathObject> SimplifyDoubleNumberSum(DoubleFloat a, Number b)
    {
      ...
      if (b is Integer) val = a.val + ((Integer)b).val;
      if (b is Fraction) val = a.val + ((Fraction)b).ToDouble().val;
      return val == 0.0 ? new List<MathObject>() : new List<MathObject> { new DoubleFloat(val) };
    }
    static List<MathObject> RecursiveSimplify(List<MathObject> elts)
    {
      if (elts.Count == 2)
      {
            ...
            (elts[1] is Integer || elts[1] is Fraction))
        {
          ...
          return P == 0 ? new List<MathObject>() : List(P);
        }
        ...
        return res == 0 ? new List<MathObject>() : List(res);
      }
      if (elts[0] is Sum)
        return
            MergeSums(
